...
> 目标版本：待定  
> 首期算法：EAGLE3  
> 首期运行模式：同步在线训练、独立 Draft 资源组、`use_logits=false`
...
- 独立 Ray Draft 资源组使用 DDP 训练 EAGLE3，并按全局有效 token 数归一化梯度；
...
- 当前强制 `Megatron + full/NCCL + PP=1 + VPP=1 + CP=1 + 非共卡`，并拒绝 MTP、routing replay、`keep-old-actor` 和有损 acceptance。
...
| 文件 | 改动 |
|---|---|
| `train.py` | 创建 Draft group/coordinator；插入 collect/train/save/publish 时序 |
| `vime/utils/arguments.py` | 新增 Draft 参数与启动校验 |
| `vime/ray/placement_group.py` | 新增 `draft` role 和 Draft group 创建函数 |
| `vime/backends/megatron_utils/actor.py` | 启用特征采集；必要时强制 old-actor 前向；返回 manifest |
...
| `vime/ray/rollout.py` | 汇总 Draft 与 speculative metrics |
| `vime/rollout/vllm_rollout.py` | 将 vLLM speculative metrics 写入 `meta_info` |
...
tests/speculative_training/
    ...
    test_eagle3_alignment.py
    test_feature_window.py
    test_target_head_version.py
    test_megatron_feature_collector.py
    test_draft_weight_mapping.py
    test_draft_weight_update.py
    test_draft_checkpoint.py
    test_external_draft_e2e.py
```
...
- Draft FSDP loss/token mean 与单卡结果一致；
...
| Draft 训练占用过多 GPU | PPO 成本升高 | 独立资源、采样预算、窗口、训练间隔、offload |
...
2. **Target 保持 Megatron，Draft 使用 HF/FSDP。** 两者通过显式 feature schema 和权重发布协议解耦。
3. **从 Actor old-logprob 前向采集。** 不扩展 vLLM HTTP 协议传输大 hidden state。
4. **首期使用独立 Draft GPU。** 优先验证正确性和稳定性，再做共卡优化。
