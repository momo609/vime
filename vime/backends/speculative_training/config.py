...
def validate_external_draft_args(args: Namespace) -> None:
    ...
    if int(getattr(args, "draft_num_nodes", 1)) <= 0 or int(getattr(args, "draft_num_gpus_per_node", 1)) <= 0:
        raise ValueError("External Draft training requires a positive Draft node/GPU count")
